...
public:
    ...
    // OPERATION ALL ITERATORS MUST PROVIDE
    ...
    // OPERATION INPUT ITERATORS MUST PROVIDE
...
public:
    ...
    // OPERATIONS ALL ITERATOR MUST PROVIDE
    // increment the iterator to the next element and
    // return a reference to the iterator
    ...
    Iterator operator++(int) noexcept {
        Iterator temp{*this};
        ConstIterator<T>::operator++(); // base-class version
        return temp;
    }
...
struct MyArray {
    ...
    [[nodiscard]] constexpr size_type size() const noexcept { return SIZE; } // return size

    // member functions that return iterators
    iterator begin() { return iterator{&m_data[0]}; }
    iterator end() { return iterator{&m_data[0] + size()}; }
    const_iterator begin() const const { return const_iterator{&m_data[0]}; }
