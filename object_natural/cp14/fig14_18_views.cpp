...
int main() {
    ...
    std::ranges::copy(evens | std::views::drop(1000) | std::views::take(5), output);

    std::cout << "\nFirst five even ints greater than 1000: ";
    std::ranges::copy(
            evens
            | std::views::drop_while([](int x) { return x <= 1000; })
            | std::views::take(5), output
    );

    // allow std::string object literals
    using namespace std::string_literals;

    std::map<std::string, int> romanNumerals{{"I"s, 1}, {"II"s, 2}, {"III"s, 3}, {"IV"s, 4}, {"V"s, 5}};
    ...
    std::cout << "\nvalues in romanNumerals via std::view::elements: ";
